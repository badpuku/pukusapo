...
async function main() {
  ...
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
    console.error('Required environment variables: SUPABASE_URL, SUPABASE_SECRET_KEY')
    process.exit(1)
  }
  if (!ENCRYPTION_KEY) {
  ...
  if (!adminProfile) {
    console.error('Admin profile not found. Please create an admin user first.')
    process.exit(1)
  }
  console.log(`Found admin profile: ${adminProfile.full_name}`)

  // Google Sheets からアカウント取得
  const config = await loadAccountsFromSheet(GOOGLE_SHEETS_ID, GOOGLE_API_KEY, GOOGLE_SHEETS_NAME)
  ...
  const newAccounts = config.accounts.filter((a) => !existingUserIds.has(a.userId))
  ...
  for (const account of newAccounts) {
    try {
      const encryptedPassword = await encryptPassword(account.password, ENCRYPTION_KEY)
